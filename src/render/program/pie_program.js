...
const pieUniforms = (context: Context, locations: UniformLocations): PieUniformsType => ({
...
const pieUniformValues = (
    painter: Painter,
    coord: OverscaledTileID,
    tile: Tile,
    layer: PieStyleLayer
): UniformValues<PieUniformsType> => {
    const transform = painter.transform;

    let pitchWithMap: boolean, extrudeScale: [number, number];
    if (layer.paint.get('pie-pitch-alignment') === 'map') {
    ...
    return {
        ...
        'u_scale_with_map': +(layer.paint.get('pie-pitch-scale') === 'map'),
        'u_matrix': painter.translatePosMatrix(
            coord.posMatrix,
            tile,
            layer.paint.get('pie-translate'),
            layer.paint.get('pie-translate-anchor')),
...
export {pieUniforms, pieUniformValues};
